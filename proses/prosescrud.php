...
class prosesCrud {
    ...
    function tampil_data_pembelian($tabel,$user_id)
    {
        $row = $this->db->prepare("SELECT * FROM $tabel where IdPembelian = ?");
        $row->execute(array($user_id));
        return $hasil = $row->fetchAll();
    }  

    function tampil_data_penjualan($tabel,$user_id)
    {
        $row = $this->db->prepare("SELECT * FROM $tabel where IdPenjualan = ?");
        $row->execute(array($user_id));
        return $hasil = $row->fetchAll();
    }    
